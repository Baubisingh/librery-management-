...
import com.sopra.service.BookService;
...
public class BookController {
	...
	public String displayBookInfo(Model model) {
		...
		if (allBooks.isEmpty())
			model.addAttribute("msg",
					"Unfortunately, the library is empty now. Let's" + "contribute by adding some books");
		else {
	...
	/*
	 * @GetMapping("/availableForm/{id}") public String availableForm(@PathVariable
	 ...
	 * 
	 * @PostMapping("/available") public String available(@RequestParam int id,
	 * Model model) {
	 * 
	 * Book book = bookRepo.findById(id).get(); book.setStatus("available");
	 * 
	 * bookRepo.save(book);
	 * 
	 * model.addAttribute("msg", "Book returned successfuly.");
	 * 
	 * return displayBookInfo(model); }
	 */
